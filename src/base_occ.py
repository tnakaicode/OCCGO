...
class plot2d (PlotBase):
    ...
    def make_patch_spines_invisible(self, axs):
    ...
    def add_twin(self, aspect="auto", side="right", out=0):
        ...
        self.make_patch_spines_invisible(axt)
...
def qtviewer_display(backend_str=None,
                     size=(1024, 768),
...
class plotocc (SetDir, Viewer):
    ...
    def __init__(self, temp=True, disp=True, touch=False):
        ...
        # OCC Viewer
        if disp == True:
            self.display, self.start_display, self.add_menu, self.add_function, self.wi = qtviewer_display()
            if touch == True:
