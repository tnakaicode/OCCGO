...
if __name__ == '__main__':
    ...
    for j in range(slitgap.size):
        ...
        position1x = numpy.linspace(-sourcesize / 2,
                                    sourcesize / 2, sourcepoints)
        tmp = numpy.linspace(-slitsize / 2, slitsize / 2, slitpoints)
        if slitgap[j] != 0:
            position2x = numpy.concatenate(
                (tmp - slitgap[j] / 2, tmp + slitgap[j] / 2))
        else:
            position2x = tmp
        position3x = numpy.linspace(-detsize / 2, detsize / 2, detpoints)

        fields12 = goFromTo(position1x, position2x,
                            slitdistance, lensF=lensF, wavelength=wavelength)
        fields23 = goFromTo(position2x, position3x, detdistance,
                            lensF=None, wavelength=wavelength)
        ...
        fieldPhase = numpy.arctan2(numpy.real(
            fieldComplexAmplitude), numpy.imag(fieldComplexAmplitude))

        # do the ensemble average
        tmpSource = numpy.exp(1.j * 2 * numpy.pi *
                              numpy.random.mtrand.rand(sourcepoints))
        ...
        for i in range(detpoints):
            out = numpy.array(
                (1e2 * position3x[i], fieldIntensity[i], fieldPhase[i], fieldIntensityEA[i]))
