...
def SRWWavefrontFromWavefront(self, wavefront, Rx, dRx, Ry, dRy, resample_x=1.0, resample_y=1.0):
    wavefront = wavefront.asEvenGridpointsGrid(
        resample_x=resample_x, resample_y=resample_y)
    s = wavefront.E_field_as_numpy()[0, :, :, 0].size

    r_horizontal_field = wavefront.E_field_as_numpy(
    )[0, :, :, 0].real.transpose().flatten().astype(np.float)
    i_horizontal_field = wavefront.E_field_as_numpy(
    )[0, :, :, 0].imag.transpose().flatten().astype(np.float)
    ...
    r_vertical_field = wavefront.E_field_as_numpy(
    )[0, :, :, 1].real.transpose().flatten().astype(np.float)
    i_vertical_field = wavefront.E_field_as_numpy(
    )[0, :, :, 1].imag.transpose().flatten().astype(np.float)
...
def numpyArrayToSRWArray(numpy_array):
    ...
    r_horizontal_field = numpy_array[:, :].real.transpose(
    ).flatten().astype(np.float)
    i_horizontal_field = numpy_array[:, :].imag.transpose(
    ).flatten().astype(np.float)
...
def SRWWavefrontFromElectricField(horizontal_start, horizontal_end, horizontal_efield,
    ...
    if horizontal_size % 2 == 1 or \
       vertical_size % 2 == 1:
        raise Exception(
            "Both horizontal and vertical grid must have even number of points")
...
def SRWEFieldAsNumpy(srw_wavefront):
    ...
    x_polarization = SRWArrayToNumpy(
        srw_wavefront.arEx, dim_x, dim_y, number_energies)
    y_polarization = SRWArrayToNumpy(
        srw_wavefront.arEy, dim_x, dim_y, number_energies)
